...
RightArm::RightArm(BLECharacteristicRegistry& characteristicRegistry):
    ...
    laserMotor(),
    laserMotorPin((int)RightArmPin_e::LaserMotor),
    rocketState(),
    rocketState_t0(),
    laserState(),
    laserState_t0(),
    repulsorLED(1, (int)RightArmPin_e::RepulsorNeoPixel, NEO_RGBW + NEO_KHZ800)
{
...
void RightArm::setup()
{
    ...
    pinMode((int)RightArmPin_e::RocketCoverServo, OUTPUT);
    pinMode((int)RightArmPin_e::RocketArmServo, OUTPUT);
...
void RightArm::attachLaserMotor()
{
...
void RightArm::detachLaserMotor()
{
    if (laserMotor.attached())
    {
        laserMotor.detach();
...
void RightArm::changeLaserState(LaserState_e newState)
{
    laserState = newState;
...
double RightArm::scaleLaserMotorAngle(double norm_motorAngle)
{
...
void RightArm::loopLaser()
{
    ...
    case LaserState_e::Off:
    {
        ...
        digitalWrite((int)RightArmPin_e::Laser, HIGH);
    ...
    bool isNotify)
{
    ...
    Serial.println( F("RightArm laser off") );
                changeLaserState(LaserState_e::Closing);
                break;
            case LaserCommands::ON:
    Serial.println( F("RightArm laser on") );
                changeLaserState(LaserState_e::Opening);
                break;
            default:
    Serial.println( F("RightArm unknown laser command") );
                // Error
                break;
            }
        }
        ...
        {
            Serial.print( F("RightArm::onValueChanged laser receiveUniqueMessage failed") );
        }
    }
    ...
    Serial.println( F("RightArm unknown rocket command") );
        ...
        {
            Serial.print( F("RightArm::onValueChanged rocket receiveUniqueMessage failed") );
        }
    }
    ...
    Serial.println( F("RightArm repulsor off") );
                repulsorLED.setBrightness(0); // off
                repulsorLED.setPixelColor(0, repulsorLED.Color(0, 0, 0, 255));
                repulsorLED.show();
                break;
    ...
    Serial.println( F("RightArm repulsor charge") );
