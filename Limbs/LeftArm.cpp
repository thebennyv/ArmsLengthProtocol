...
LeftArm::LeftArm(BLECharacteristicRegistry& characteristicRegistry):
    ...
    laserMotor(),
    laserMotorPin((int)LeftArmPin_e::LaserMotor),
    rocketState(),
    rocketState_t0(),
    laserState(),
    laserState_t0(),
    repulsorLED(1, (int)LeftArmPin_e::RepulsorNeoPixel, NEO_RGBW + NEO_KHZ800)
{
...
void LeftArm::setup()
{
    ...
    pinMode((int)LeftArmPin_e::RocketCoverServo, OUTPUT);
    pinMode((int)LeftArmPin_e::RocketArmServo, OUTPUT);
    pinMode((int)LeftArmPin_e::LaserMotor, OUTPUT);
    pinMode((int)LeftArmPin_e::Laser, OUTPUT);
    digitalWrite((int)LeftArmPin_e::Laser, HIGH); // off
    ...
    repulsorLED.begin();
    repulsorLED.setBrightness(0); // off
    repulsorLED.setPixelColor(0, repulsorLED.Color(0, 0, 0, 255));
    repulsorLED.show();

    changeRocketState(RocketState_e::ClosingRocket);
    changeLaserState(LaserState_e::Off);
}
...
void LeftArm::attachLaserMotor()
{
    if (!laserMotor.attached())
    {
        laserMotor.attach(laserMotorPin);
    }
}

void LeftArm::detachLaserMotor()
{
    if (laserMotor.attached())
    {
        laserMotor.detach();
...
void LeftArm::changeLaserState(LaserState_e newState)
{
    laserState = newState;
    laserState_t0 = millis();
}
...
double LeftArm::scaleLaserMotorAngle(double norm_motorAngle)
{
    const double outputMin = (double)LaserMotorAngles_e::Min;
    const double outputMax = (double)LaserMotorAngles_e::Max;
    const double outputRange = outputMax - outputMin;

    return (norm_motorAngle * outputRange) + outputMin;
}
...
void LeftArm::loopLaser()
{
    ...
    const unsigned long dt = currentTime - laserState_t0;

    switch (laserState)
    {
    case LaserState_e::Off:
    {
        // todo should we periodically attach and pulse the servos to ensure things stay put?
        detachLaserMotor();

        digitalWrite((int)LeftArmPin_e::Laser, HIGH);

        break;
    }
    case LaserState_e::Opening:
    {
        const double norm_dt = (double)dt / (double)LaserStateDurations_e::DurationOpening;
        if (norm_dt <= 1.0)
        {
            attachLaserMotor();
            laserMotor.write((int)LaserMotorAngles_e::Max);
        }
        if (norm_dt >= 1.0)
        {
            changeLaserState(LaserState_e::On);
        }
        break;
    }
    case LaserState_e::On:
    {
        // todo should we periodically attach and pulse the servos to ensure things stay put?
        detachLaserMotor();

        digitalWrite((int)LeftArmPin_e::Laser, LOW); // on

        break;
    }
    case LaserState_e::Closing:
    {
        digitalWrite((int)LeftArmPin_e::Laser, HIGH); // off

        const double norm_dt = (double)dt / (double)LaserStateDurations_e::DurationClosing;
        if (norm_dt <= 1.0)
        {
            attachLaserMotor();
            laserMotor.write((int)LaserMotorAngles_e::Min);
        }
         if (norm_dt >= 1.0)
        {
            changeLaserState(LaserState_e::Off);
    ...
    bool isNotify)
{
    ...
    Serial.println( F("LeftArm laser off") );
                changeLaserState(LaserState_e::Closing);
                break;
            case LaserCommands::ON:
    Serial.println( F("LeftArm laser on") );
                changeLaserState(LaserState_e::Opening);
                break;
            default:
    Serial.println( F("LeftArm unknown laser command") );
                // Error
                break;
            }
        }
        ...
        {
            Serial.print( F("LeftArm::onValueChanged laser  receiveUniqueMessage failed") );
        }
    }
    ...
    Serial.println( F("LeftArm unknown rocket command") );
        ...
        {
            Serial.print( F("LeftArm::onValueChanged rocket receiveUniqueMessage failed") );
        }
    }
    ...
    Serial.println( F("LeftArm repulsor off") );
                repulsorLED.setBrightness(0); // off
                repulsorLED.setPixelColor(0, repulsorLED.Color(0, 0, 0, 255));
                repulsorLED.show();
                break;
    ...
    Serial.println( F("LeftArm repulsor charge") );
                repulsorLED.setBrightness(255); // full blast
                repulsorLED.setPixelColor(0, repulsorLED.Color(0, 0, 0, 255));
                repulsorLED.show();
                break;
